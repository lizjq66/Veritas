...
\paragraph{Arithmetic basis.} In v0.1 Veritas compiled against
Lean~4's opaque \texttt{Float} type and carried twenty IEEE~754 axioms
covering ordering, sign preservation, and rounding monotonicity.
v0.2 migrates the entire decision-path arithmetic to exact
\texttt{Rat} via Mathlib~\cite{mathlib}. After the migration,
\textbf{every gate-level and underlying theorem depends only on Lean's
three foundational axioms} — \texttt{propext}, \texttt{Classical.choice},
and \texttt{Quot.sound} — the same basis Mathlib itself rests on. No
Veritas-specific axiom remains, no rounding assumption, no Float
...
in Table~\ref{tab:thms}. All proofs close against Mathlib's exact
\texttt{Rat} arithmetic; no IEEE 754 rounding axioms are invoked.
...
\paragraph{v0.2 deliverables.} The \texttt{Verifier} Python surface, the
...
for the same input, enforced by 114 passing tests with no mocks on the
kernel. \texttt{lake build} produces the compiled kernel binary with
zero \texttt{sorry}. The decision-path arithmetic runs on exact
\texttt{Rat}; no \texttt{Veritas}-specific axioms exist.
