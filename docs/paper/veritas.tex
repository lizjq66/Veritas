...
$1.0$, unknown cross-asset pairs count at $0.0$, and explicit
entries override both defaults. Direction conflicts are restricted
to same-asset positions, so an opposite-direction position on an
uncorrelated asset (a potential hedge) is no longer falsely flagged
as a conflict.
...
When Gate~1 approves a proposal against a multi-policy registry it
attaches the union of assumptions from every firing strategy. Gate~2,
in contrast, consumes a scalar \texttt{(reliability, sampleSize)}
pair. The learning layer bridges the two with an
\texttt{aggregateReliability} function that takes the element-wise
minimum across its inputs. Three supporting theorems pin down the
function's shape: \texttt{aggregateReliability\_empty} (empty list
returns the no-data default \texttt{(0.5, 0)}),
\texttt{aggregateReliability\_singleton} (single-element lists are
pass-through), and \texttt{aggregateReliability\_sampleSize\_le\_head}
(the aggregate sample size never exceeds any input's total, so a
