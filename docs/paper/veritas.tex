...
acting under, does the proposal cohere? In particular: does the direction
match what a declared policy would emit here, and does the proposal carry
well-formed, non-empty assumptions? Gate~1 forces the caller to be
explicit about what it is betting on. On approval, Gate~1 attaches the
assumptions Veritas's policy associates with the signal, so downstream
auditors see \emph{what was bet on}.
...
v0.1 catches opposite-direction conflicts and enforces a gross-notional
cap; v0.2 will handle multi-asset correlation once a second policy is
active.
...
1. Signal consistency
  ...
  & \textsc{approve} / \textsc{reject} \\
2. Constraint compatibility
  & Size fits the reliability-adjusted ceiling?
  & \textsc{approve} / \textsc{resize} / \textsc{reject} \\
3. Portfolio interference
  & Clashes with existing positions? Breaches cap?
  & \textsc{approve} / \textsc{resize} / \textsc{reject} \\
\bottomrule
\end{tabular}
...
25\%-cap, zero-at-no-edge, monotone-in-reliability, exploration-cap-at-1\%).
...
\midrule
\texttt{verifySignal\_approve\_implies\_consistent}
  & Gate~1 \textsc{approve} $\Rightarrow$ proposal is signal-consistent. \\
\texttt{checkConstraints\_approve\_within\_ceiling}
  & Gate~2 \textsc{approve} $\Rightarrow$ $p.\texttt{notional} \leq \texttt{ceiling}$. \\
\texttt{checkConstraints\_resize\_respects\_ceiling}
  & Gate~2 \textsc{resize}~$n$ $\Rightarrow$ $n \leq \texttt{ceiling}$. \\
\texttt{checkPortfolio\_approve\_respects\_cap}
  & Gate~3 \textsc{approve} $\Rightarrow$ gross notional under cap. \\
\texttt{certificate\_soundness}
  & \texttt{approves} $= \text{true}$ $\Rightarrow$ Gate~1 consistent $\wedge$ Gate~2/3 non-reject. \\
\bottomrule
\end{tabular}
...
For every $p, c, \text{port}$, if
$\texttt{emitCertificate}(p, c, \text{port}).\texttt{approves} = \text{true}$
then:
\begin{enumerate}[leftmargin=2em, itemsep=1pt, topsep=2pt]
\item $\texttt{signalConsistent}(p)$ holds;
\item the certificate's \texttt{gate2} verdict is not a \textsc{reject};
\item the certificate's \texttt{gate3} verdict is not a \textsc{reject}.
\end{enumerate}
\end{theorem}
...
Activations beyond v0.1 are:

\begin{description}[leftmargin=2em, itemsep=2pt, topsep=2pt]
\item[v0.2.] A second policy (basis, liquidation-cascade reversal, or
  cross-exchange perp--perp basis; candidates under evaluation). Gate~1
  dispatches over a policy registry; Gate~3 gains multi-asset
  correlation. Concurrently: migrate numeric decision paths off
  \texttt{Float} where practical to eliminate rounding-dependent axioms.
\item[v0.3.] Assumption library exposed as a read-only oracle.
  Certificates carry a hash of the Lean theorem they rest on, so
  callers can verify the trust claim without running the kernel
  themselves.
\item[v0.4+.] Public registry of policies and theorems; cross-asset
  policies; first cross-agent integrations.
\end{description}

Each step adds surface and therefore costs trust. The default is not to
take the step until the previous step's trust is deployed.
...
legible gate than a large, opaque trader. The v0.1 product is the
smallest such gate we could ship while keeping all three questions on
the critical path.
